# -*- coding: utf-8 -*-
"""Step 2: Add 12 new SKUs (6 wedding-invitations + 6 place-cards) to products.ts.
Strategy: insert before the final `];` at line 20411.
Each SKU: compact structure (no longDescription for v1; can be added later).
"""
import os
...
# 6 place-cards SKUs (PC-001 to PC-006)
PLACE_CARDS_SKUS = '''
...
# Insert before the final `];` at line 20411
# Find: `},\n];\n` (the end of the products array)
# Replace with: `},\n<NEW_SKUS>\n];\n`
# Actually, the last entry is followed by `},\n];` (line 20409, 20411)
# So insert NEW_SKUS after line 20409's `},` and before line 20411's `];`

old_end = "},\n];\n"
new_end = "},\n" + WEDDING_SKUS + PLACE_CARDS_SKUS + "\n];\n"

# Find the LAST occurrence of `};\n` in the file
last_closing = src.rfind("];\n")
if last_closing == -1:
    raise Exception("Could not find `];` at end of products array")

# The `];` is preceded by `}` (closing of the last product)
# Insert before `];\n`
# Find the `,\n];` pattern just before
# Actually, look for `},\n];` at the end

# Check if old pattern exists
if src[last_closing-3:last_closing] == "},\n":
    new_src = src[:last_closing-3] + "},\n" + WEDDING_SKUS + PLACE_CARDS_SKUS + "\n];\n" + src[last_closing+3:]
else:
    # Try just `];` at very end
    new_src = src[:last_closing] + WEDDING_SKUS + PLACE_CARDS_SKUS + "\n];\n" + src[last_closing+3:]
...
print(f"Wedding SKUs: WI-001 to WI-006 (6 SKUs)")
print(f"Place Cards SKUs: PC-001 to PC-006 (6 SKUs)")
