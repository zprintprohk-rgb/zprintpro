"""
V3.7 DoD 6: wedding-invitations Pillar 加厚 + 补全 en/ja 翻译 (v2 修 depth tracking)
"""
...
# 1. 找 weddingInvitationsContent 块 (depth=0 结束)
START_IDX = 4601  # L4602 'const weddingInvitationsContent' 的 0-indexed
...
for i in range(START_IDX, END_IDX):
    if lines[i].strip().startswith("'zh-hk':"):
        zh_start = i
        break
...
# 3. 删除原 zh-hk 子块 (L4603-L4693), 替换为新 3 locale 块
new_3locales = ZH_HK_BLOCK + '\n\n' + EN_BLOCK + '\n\n' + JA_BLOCK

# lines[zh_start:zh_end+1] 是原 zh-hk 块
# 我们要保留 lines[:zh_start] + new_3locales.split('\n') + lines[zh_end+1:]
new_lines = lines[:zh_start] + new_3locales.split('\n') + lines[zh_end+1:]

# 4. 写回
PATH.write_text('\n'.join(new_lines), encoding='utf-8')
print(f'  ✅ category-seo-content.ts 写入')
...
wedding_block_start = new_content.find('weddingInvitationsContent')
wedding_block_end = new_content.find('greetingCardsContent', wedding_block_start)
...
for loc, content_part in [
    ...
    if loc == 'zh-hk':
        ...
        print(f'  {loc}: {size} chars (期望 6000+)')
    else:
        words = len(content_part.split())
        print(f'  {loc}: {words} words (期望 en 600-900 / ja 400-600)')
