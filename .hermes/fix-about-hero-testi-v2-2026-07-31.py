# -*- coding: utf-8 -*-
"""
fix-about-hero-testi-v2-2026-07-31.py
K3 11:37 + 11:42 拍板 A: Hero + 12 Tier A 行业 icon 卡片
"""
import io
import sys
import re
...
# 用 regex 匹配, 避免 escape 麻烦
old_hero_pat = re.compile(
...
new_hero = '''        {/* 2026-07-31 K3 拍板 A: Hero 改用 factory-banner.webp 背景 + 深色蒙层 */}
...
m = old_hero_pat.search(content)
assert m, 'Hero not found'
content = old_hero_pat.sub(new_hero, content, count=1)
print('[fix1] Hero -> factory-banner.webp background + dark overlay')

# === fix 2: zh-hk testimonialSubtitle + testimonials ===
...
# === fix 5: Testimonials section 渲染用 industries ===
# 找 testimonials section 块
old_sec_pat = re.compile(
    r"        \{/\* 2026-07-30 K4 拍板 2: Testimonials 3 段 mock \(K3 拍图后替换为真实客户\) \*/\}.*?</section>\n",
    re.DOTALL
)
m = old_sec_pat.search(content)
assert m, 'Testimonials section not found'

new_sec = """        {/* 2026-07-31 K3 拍板 A: 12 Tier A 行业 icon 卡片 (删 MOCK 占位, 不走网上搜真实 logo) */}
...
"""
content = old_sec_pat.sub(new_sec, content, count=1)
print('[fix5] Testimonials section -> 12 industries grid')

# === fix 6: 在 component 加 INDUSTRY_ICONS map ===
# 找 "const t = translations[locale];" 之前
icon_map = """  // 2026-07-31 K3 拍板 A: 12 行业 icon map (zh-hk/en/ja 共享 icon, 名字各自本地化)
...
old_t = "  const t = translations[locale];\n"
new_t = icon_map
assert old_t in content
content = content.replace(old_t, new_t, 1)
print('[fix6] component 加 INDUSTRY_ICONS map + 改 const t')
