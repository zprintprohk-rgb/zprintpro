...
# -*- coding: utf-8 -*-
"""
Step 3.3: Insert poster-size-guide entries into src/app/[locale]/blog/[slug]/page.tsx
Precise insertion by block boundaries:
- zh-hk block: starts at "  'zh-hk': {", ends at first "\n  },\n"
- en block: starts at "  en: {", ends at first "\n  },\n"
- ja block: starts at "  ja: {", ends at "\n  }\n};"
- articleSlugs: insert before "];" at end of array (after same-day-flyers line)
"""
...
assert zh_start != -1, 'zh-hk block not found'
zh_end_marker = content.find('\n  },\n', zh_start)
assert zh_end_marker != -1, 'zh-hk end not found'
zh_end = zh_end_marker + len('\n  },\n')
content = content[:zh_end] + ZH_ENTRY + content[zh_end:]

# --- en block (re-find after insertion) ---
en_start = content.find('\n  en: {\n')
assert en_start != -1, 'en block not found'
en_end_marker = content.find('\n  },\n', en_start)
assert en_end_marker != -1, 'en end not found'
en_end = en_end_marker + len('\n  },\n')
content = content[:en_end] + EN_ENTRY + content[en_end:]

# --- ja block ---
ja_start = content.find('\n  ja: {\n')
assert ja_start != -1, 'ja block not found'
ja_end_marker = content.find('\n  }\n};', ja_start)
assert ja_end_marker != -1, 'ja end not found'
ja_end = ja_end_marker + len('\n  }\n};')
content = content[:ja_end] + JA_ENTRY.rstrip('\n') + '\n' + content[ja_end:]

# --- articleSlugs: insert before final "];" of the array ---
# find the articleSlugs declaration end: the "];" that closes the array
