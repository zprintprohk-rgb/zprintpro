...
> ① **段 3 主段锚 H2 或 H3 均可**（判据 = (H2+H3) 总数 ≥6 且问句式 ≥50%（含边界·K3 裁决 iii）；关键在「层级逻辑清晰」，不强求 H2；**不要为迁就门禁去动已上线 DOM 结构**）。
...
> ① **段位口径锁定**：深度 blog 段位标准 = **12 段骨架**（SSoT §3.1），**不是 9 段**（9 段已于 9/3 撤回），**V5.1 只是渲染层「快速答案块」样式标记、与段数无关**——写作前不要再被这两条旧口径干扰。
> ② **段 9 FAQ 格式必须是生产正则可解析的形态**：`<p><strong>Q1: 问题</strong><br/>A: 答案</p>`（`<p>` **不得带 class**）。实测教训：`<p class="mb-3">` 或 `<li><strong>问?</strong>答</li>` 形态 ⇒ `extractFaqFromHtml` 解析不出 ⇒ `faqJsonLd = null` ⇒ **线上静默失去 FAQPage**（2026-09-19 实测 12/15 个 Pillar-locale 组合踩坑）。交付前必跑 `node scripts/guards/blog-quality-12-rules-guard.js --online` 并确认段 9 + 段 12 均 PASS。
