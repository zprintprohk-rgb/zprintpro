...
# -*- coding: utf-8 -*-
"""
K3 B 指令 67-B 25 词清单剩余 22 词 (8/17 10:35 拍板).
- 已落 3 词 (menus/calendars/flyers.ja) 在 7481e51.
- 即日印刷 (rush-printing-delivery 服务页) 留 8/18 续做.
- 本期攒批 21 词 → 7 个 slug (6 类目 + 1 服务页).
- 不 commit/push, 仅生成 diff 报告, 等 K3 拍板.

改写模式 (per 7481e51):
- zh-hk: 机会词 + 卖点 (防水/数量/价格/速度) | + CTA + 品牌
- en: 机会词 + from $XX | + Free Shipping/其他 | ZprintPro
- ja: 机会词 | 価格/数量/納期 | ZprintPro
"""
import json
import os
import sys

# PowerShell GBK 编码: 强制 UTF-8 输出
sys.stdout.reconfigure(encoding='utf-8', errors='replace')

ROOT = r'F:\zprintpro-nextjs'
SEO = os.path.join(ROOT, 'src', 'lib', 'seo.ts')
OUT = os.path.join(ROOT, '.hermes', '_67b_22_seo_diff.md')
OUT_JSON = os.path.join(ROOT, '.hermes', '_67b_22_seo_diff.json')

# 7 个 slug 21 词 改写 (3 locale × 3 字段 = 9 字段/slug)
DIFFS = [
    {
        'slug': 'stickers',
        'gsc_words_covered': ['貼紙印刷', '貼紙', '透明貼', 'small quantity label printing'],
        'mode': '防水 + 小批量 + 異形 + 燙金',
        'titles': {
    ...
    {
        'slug': 'posters',
        'gsc_words_covered': ['海報印刷', '印海報', 'a2 prints', 'a1 poster size'],
        'mode': 'A0-A2 + 防水 + 1 張起',
        'titles': {
    ...
    {
        'slug': 'books',
        'gsc_words_covered': ['saddle stitch booklet', 'saddle stitch booklets', '印書', '騎馬釘書刊', '騎馬釘', '教材 印刷製本'],
        'mode': '騎馬釘 + 膠裝 + 精裝 + 50本起',
        'titles': {
    ...
    {
        'slug': 'banners',
        'gsc_words_covered': ['戶外橫額'],
        'mode': '易拉寶 + X架 + 車身廣告 + 防水 UV',
        'titles': {
    ...
    # NOTE: rush-printing-delivery 服务页 (印刷 カラー cmyk / 印刷 カラー モード + 即日印刷)
    # 不在 src/lib/seo.ts (在 services/rush-printing-delivery/page.tsx), K3 8/18 续做.
    # 本期 6 类目 = 20 词. K3 22 词剩余 = 20 词本期 (6 slug) + 1 词即日印刷 (8/18 续) + 1 词印刷 cmyk (8/18 续).
]
...
def main():
    ...
    # 写 Markdown 报告
    total_words = sum(len(d['gsc_words_covered']) for d in DIFFS)
    slugs = [d['slug'] for d in DIFFS]
    with open(OUT, 'w', encoding='utf-8') as f:
        f.write(f'# 67-B 21 词 SEO 攒批 diff (K3 8/17 10:35 拍板, 本地未 commit)\n\n')
        f.write(f'> **攒批状态**: 本地落盘未 commit，等 K3 拍板 22 词具体清单 + 实施节奏\n')
        f.write(f'> **覆盖范围**: {len(DIFFS)} 个 slug × 3 locale × 3 字段 = {len(DIFFS)*9} 处改动\n')
        f.write(f'> **GSC 词覆盖**: {total_words} 词 (8/5 25 词 + 8/14 15 词合并去重, 扣除 4 金矿已落 3 + 即日印刷留 8/18 + 咭片禁区 2 词)\n')
        f.write(f'> **未覆盖**: 即日印刷 (rush-printing-delivery 服务页 K3 8/18 续做, 共 1 词 = 凑齐 K3 22 词)\n\n')
        f.write('---\n\n')

        for d in DIFFS:
            f.write(f'## {d["slug"]} ({len(d["gsc_words_covered"])} 词)\n\n')
            f.write(f'**GSC 词**: {", ".join(d["gsc_words_covered"])}\n\n')
            f.write(f'**模式**: {d["mode"]}\n\n')
            f.write('### titles\n\n')
            for loc in ['zh-hk', 'en', 'ja']:
                f.write(f'- **{loc}**: `{d["titles"][loc]}`\n')
            f.write('\n### keywords\n\n')
            for loc in ['zh-hk', 'en', 'ja']:
                kw_count = len([k for k in d['keywords'][loc].split(',') if k.strip()])
                f.write(f'- **{loc}** ({kw_count} 词): `{d["keywords"][loc]}`\n')
            f.write('\n### descriptions\n\n')
            for loc in ['zh-hk', 'en', 'ja']:
                f.write(f'- **{loc}** ({len(d["descriptions"][loc])} chars):\n  > {d["descriptions"][loc]}\n')
            f.write('\n---\n\n')

        f.write('\n## 待 K3 拍板\n\n')
        f.write('1. **22 词具体清单**: 21 词本期攒批 + 1 词 (即日印刷服务页) 留 8/18 续, 是否一致?\n')
        f.write('2. **实施节奏**: 攒批 1 push (今日 push 1/5, 月 15/150) 还是分批 2 push (8/18 + 8/19)?\n')
        f.write('3. **rush-printing-delivery 服务页**: 8/18 续做 = 周一晚, 是否同意?\n')
        f.write('4. **咭片禁区词**: 印咭片 / 咭片 已跳过 (per §11 主营品类约束), 是否接受?\n\n')
        f.write('## 验收 SOP (push 前必跑)\n\n')
        f.write('1. `node scripts/check-encoding.js --fix` (UTF-16/CRLF 守门)\n')
        f.write('2. `node scripts/check-i18n.js` (8 locale 完整)\n')
        f.write('3. `node scripts/check-简体字.js` (per K3 §0.5 简体字守门)\n')
        f.write('4. `npx tsc --noEmit` (TypeScript 0 error)\n')
        f.write('5. `npm run build` (本地构建 PASS, 节省 CF build 18 min)\n')
        f.write('6. push 后 5 步真 verify (push ahead 0 / HEAD=新 SHA / 3 URL curl 200 / sitemap mtime / JSON-LD schema)\n\n')
    print(f'✓ wrote {OUT}')
    print(f'覆盖 {total_words} 词 / {len(DIFFS)} slug / {len(DIFFS)*9} 处改动')
