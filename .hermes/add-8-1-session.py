#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Add 8/1 cron_session entry to matrix.json using standard schema."""
import json

with open('.hermes/industry-keyword-matrix.json', 'r', encoding='utf-8') as f:
    matrix = json.load(f)

# Check if 8/1 session already exists
sessions = matrix.get('v7_cron_sessions', [])
already = any(s.get('date') == '2026-08-01' for s in sessions)
if already:
    print('8/1 session already exists, skipping add')
else:
    new_session = {
        ...
        'build_quota': 0,
        'strategy': 'v7 daily 1 push 计划 (8/1 是新一天, f5700f9 7/31 已 PASS, build 健康; 0 push 攒批等 §0.6 verify-deploy + 0 candidate 8/12 复盘准备)',
        'skus': [
