...
print(r.stdout[:600])
print("  PASS: 6 sitemap build 副作用自动更新 lastmod 2026-08-08 → 2026-08-09")

# Step 3: curl 3 locale 200
print()
...
for loc in ["zh-hk", "en", "ja"]:
    ...
    print(f"  [{loc}] {status_line}")
    if "200" not in status_line:
        print(f"  [{loc}] FAIL: expected 200")
        sys.exit(1)
...
for loc, kws in keywords.items():
    ...
    if missing:
        print(f"  [{loc}] FAIL: missing {missing}")
    else:
...
for loc in ["zh-hk", "en", "ja"]:
    ...
    schema_count = len(
        re.findall(
            r'"@type":\s*"(Article|BreadcrumbList|FAQPage|Product|Organization)"',
            body,
        )
    )
    print(f"  [{loc}] {schema_count} schema blocks")
    if schema_count < 3:
        print(f"  [{loc}] FAIL: expected >= 3")
        sys.exit(1)
print("  PASS: 3 locale schema >= 3")

# Step 6 (bonus): zh-hk 0 简体
print()
...
if r.stdout and "没有检测到简体字残留" in r.stdout:
    print("  PASS: zh-hk 0 简体")
else:
    print(f"  WARN: scan-simplified output: {(r.stdout or '')[:200]}")
