...
# -*- coding: utf-8 -*-
"""
V3.10 作战包 1: 新建 /en/services/catalog-printing-china/ 大单着陆页
- 三锤: 30秒 AI 报价 + MOQ 50 + DHL 2-4 天
- 信任锤: 出货视频验货 (D4 ②层 Trustpilot 待 K3)
- §13.10 重要修订: china 词群着陆页必须正大光明写 Shenzhen factory
- 内链: china 词群 5 变体 + bulk/wholesale + /category/packaging/ + /product/ 相关 SKU
- alt 规则: alt = {主词} - {规格} | {品牌}
- Schema: Service + Product + FAQPage

3 locale 都支持, en 重点 (K3 v3.10 拍板 china 词群主要在美国市场)
"""
from pathlib import Path

OUT = Path(r"F:\zprintpro-nextjs\src\app\[locale]\services\catalog-printing-china\page.tsx")
OUT.parent.mkdir(parents=True, exist_ok=True)

# 使用 rush-printing-delivery 模板结构 + china catalog 特定内容
...
export default function CatalogPrintingChinaPage({ params }: Props) {
  ...
  const serviceJsonLd = generateServiceJsonLd(
    ...
    t.heroTitle,
    t.heroSubtitle,
    `${localePrefix}/services/catalog-printing-china/`
  );
  const productJsonLd = {
...
print(f"OK: catalog-printing-china/page.tsx created ({len(content)} chars)")
print(f"  3 locale 全: zh-hk / en / ja")
print(f"  §13.10 修订: Shenzhen factory 正大光明写")
print(f"  Schema: Service + Product + FAQPage")
print(f"  alt 规则: {{主词}} - {{规格}} | {{品牌}}")
print(f"  内链: /category/books/ + /category/packaging/ + /category/flyers/")
