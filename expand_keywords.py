...
with open('src/data/product-seo.ts', 'r', encoding='utf-8') as f:
    content = f.read()
...
for slug, exp in expansions.items():
    ...
    zh_match = re.search(r"'zh-hk':\s*'([^']+)'", block)
    en_match = re.search(r"'en':\s*'([^']+)'", block)
    ja_match = re.search(r"'ja':\s*'([^']+)'", block)
    
    if not zh_match or not en_match or not ja_match:
        continue
    
    cur_zh = zh_match.group(1)
    cur_en = en_match.group(1)
    cur_ja = ja_match.group(1)
    
    # Merge
    new_zh = ','.join(dict.fromkeys(cur_zh.split(',') + exp['zh'].split(',')))
    new_en = ','.join(dict.fromkeys(cur_en.split(',') + exp['en'].split(',')))
    new_ja = ','.join(dict.fromkeys(cur_ja.split(',') + exp['ja'].split(',')))
    
    # Build new block
    new_block = block.replace(f"'zh-hk': '{cur_zh}'", f"'zh-hk': '{new_zh}'")
    new_block = new_block.replace(f"'en': '{cur_en}'", f"'en': '{new_en}'")
    new_block = new_block.replace(f"'ja': '{cur_ja}'", f"'ja': '{new_ja}'")
    
    content = content.replace(block, new_block, 1)
    updated += 1

with open('src/data/product-seo.ts', 'w', encoding='utf-8') as f:
    f.write(content)

print(f'Updated {updated} SKU keyword entries')
