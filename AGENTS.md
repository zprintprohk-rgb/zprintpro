...
## §0.29 K3 8/30 「标题长尾 3 筛选 + 分层布局」 全站 SOP (2026-08-30 05:00 K3 拍板, 跨项目 P0 · 必查规则)
...
**§0.29.2 分层布局法** (K3 8/30 拍板, K3 9/1 02:58 升级半角当量口径 + 字符体检):
...
**§0.29.7 §0.29 教训固化源头**:
- **拍板来源**: K3 8/30 05:00 "标题长尾 3 筛选 + 分层布局" 战略拍板 (V1.1 §1.3 + V3.5 词分层延伸) + **K3 9/1 02:54 单品牌分层升级** (zh-hk = 智印港 / en/ja = ZprintPro, 双品牌不再同时出现) + **K3 9/1 02:58 GLM 拍板升级** (半角当量口径 + 字符体检 3 行 + 跨语言污染校验, 引用三源数据 Zyppy 8万/Portent 147万/2025 Q1 76% 重写率 + Vistaprint 竞品 + 香港本地标准)
- **AGENTS.md 写入**: 2026-08-30 05:45 M3 当前 turn 立即固化 + 2026-09-01 02:58 升级半角当量 + 字符体检 3 行 (per §0.28.12 教训固化源头的 K3 战略大块必固化原则)
- **数据诚信** (§0.23): 13 品类 + 10 specs 长尾候选全部过 GSC 实证筛选 (gsc_data.csv 463 rows 8/28 + gsc_page_query.csv 2294 rows 8/26) + food-boxes en/ja 跨语言污染 P0 Bug 修复 (删繁中字形, 恢复 ja FSC認証锚点)
- **跨项目 P0**: 任何 B2B 项目 title 改动必查本 SOP, 跨 session 永久生效
- **后续**: M3 必按 §0.29.2 分层布局 + §0.29.2.1 半角当量 + §0.29.2.2 字符体检 3 行 + §0.29.3 严禁 实施, 必不在 title 改动时违反 3 筛选必过铁律 + 跨语言污染零容忍
