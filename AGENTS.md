...
**§0.29.2 分层布局法** (K3 8/30 拍板, title 放不下的长尾有去处):

| 位置 | 数量 | 类型 | 验证 |
|------|------|------|------|
| **title** | 1 长尾 | 主词 + 长尾 + 数字钩子 + 品牌 | 字符数 50-60 |
...
**§0.29.4 food-boxes 实证落地** (K3 8/30 方案 A, K3 拍板优先):
- **zh-hk title**: 食品包裝印刷 **禮盒訂製** 100個起 HK$4起 | 智印港 ZprintPro (33 字符, 主词+禮盒訂製 长尾+双数字钩子+品牌)
...
**§0.29.7 §0.29 教训固化源头**:
- **拍板来源**: K3 8/30 05:00 当前 turn "标题长尾 3 筛选 + 分层布局" 战略拍板 (V1.1 §1.3 + V3.5 词分层延伸)
- **AGENTS.md 写入**: 2026-08-30 05:45 M3 当前 turn 立即固化 (per §0.28.12 教训固化源头的 K3 战略大块必固化原则)
- **数据诚信** (§0.23): 13 品类 + 10 specs 长尾候选全部过 GSC 实证筛选 (gsc_data.csv 463 rows 8/28 + gsc_page_query.csv 2294 rows 8/26)
- **跨项目 P0**: 任何 B2B 项目 title 改动必查本 SOP, 跨 session 永久生效
- **后续**: M3 必按 §0.29.2 分层布局 + §0.29.3 严禁 实施, 必不在 title 改动时违反 3 筛选必过铁律
