// K3 v3.16 G1《HK Print Inquiry Index》Vol.1 页面 (重做)
// 路由: /[locale]/insights/hk-print-inquiry-index/
// K3 F0 修复版: 不再用 next/script (CF Pages 0s 秒杀 根因), 加 generateStaticParams (3 locale)
// 业务: 8/28 中检后跑 SQL 实算注入; 当前 n=31 baseline (8/22 06:39 008 端到端实证)
...
export default async function HkPrintInquiryIndexPage({ params }: { params: { locale: string } }) {
  ...
  // Schema.org JSON-LD: Report + Dataset + FAQPage 三合一 (用原生 script 标签, 不用 next/script)
  const reportSchema = {
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-12">
      ...
      {/* Schema.org JSON-LD - 用原生 script 标签, 不用 next/script (K3 F0 修复) */}
      <script
