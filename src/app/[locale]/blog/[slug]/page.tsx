...
const posts: Record<string, Record<string, { title: string; description: string; date: string; category: string; content: string }>> = {
  'zh-hk': {
    ...
    // 2026-08-27 K3 拍板修复: 3 篇 W1+W3 zh-hk content 迁移到 src/data/blog-data/zh-hk.json (跟 A+ poster-printing-price-guide 模式一致), page.tsx 只留 metadata stub
    // 修复前: 9 段简版 + 缺 callout + 缺 table + 缺 JSON-LD FAQPage + 排版单调
    // 修复后: zh-hk 800-1000 字 9 段 + 4 FAQ + 5 内链 + 2 callout (重點摘要+數據洞察) + 2 table (价格+服务) + 0 图片 + zh-hk native
    // F0 红线: 不删 SKU/文案/长文本字段, 修复 = 迁移 + 增强
