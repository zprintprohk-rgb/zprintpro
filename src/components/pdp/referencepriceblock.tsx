 ...
 * v13 任务卡 2026-07-25
 *
 * 两件套: SSR 静态价格表 (第一个 config 的全量 tiers) + 客户端参数选择器 (材料/盒型/数量联动)
 */
...
function getShippingText(weightKg: number | null, locale: string): string {
  if (weightKg === null || weightKg === undefined) {
    return locale === "ja" ? "配送料別途" : locale === "en" ? "Shipping calculated separately" : "運費另計";
  }
  if (weightKg > 30) {
    return locale === "ja" ? ">30kg 配送料別途見積" : locale === "en" ? ">30kg freight quoted separately" : ">30kg 物流另行報價";
  }
...
export default function ReferencePriceBlock({ data, locale, whatsappNumber }: Props) {
  ...
  const selectedTier = selectedQty !== null
    ? currentConfig.tiers.find((t) => t.qty === selectedQty)
    : null;
  ...
  const waMessage = selectedTier && selectedQty
    ? buildWhatsAppMessage(
        locale,
        data.productName[locale] || "",
        currentConfig.label[locale] || "",
        selectedQty,
        selectedPrice,
        CURRENCY_CODE[locale] || "HKD"
      )
    : "";
  const waUrl = `https://wa.me/${whatsappNumber || "+8618126380255"}?text=${encodeURIComponent(waMessage)}`;
    ...
    <div className="mt-8 mb-10 border border-gray-200 rounded-lg bg-white">
      ...
      {data.configs.length > 1 && (
        <div className="px-6 py-4 border-b border-gray-100">
          ...
          <select
            className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            value={configIndex}
            onChange={(e) => { setConfigIndex(Number(e.target.value)); setSelectedQty(null); }}
          >
            {data.configs.map((cfg, i) => (
              <option key={i} value={i}>{cfg.label[locale] || `Config ${i + 1}`}</option>
            ))}
          </select>
        </div>
      )}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left">
              <th className="px-6 py-3 font-medium text-gray-700">
                {locale === "ja" ? "数量" : locale === "en" ? "Quantity" : "數量"}
              </th>
              <th className="px-6 py-3 font-medium text-gray-700">
                {locale === "ja" ? "参考単価" : locale === "en" ? "Unit Price" : "參考單價"}
              </th>
              <th className="px-6 py-3 font-medium text-gray-700 hidden sm:table-cell">
                {locale === "ja" ? "配送" : locale === "en" ? "Shipping" : "運送"}
              </th>
              <th className="px-6 py-3 font-medium text-gray-700 hidden sm:table-cell">
                {locale === "ja" ? "総額" : locale === "en" ? "Total" : "總額"}
              </th>
          ...
          <tbody className="divide-y divide-gray-100">
            {currentConfig.tiers.map((tier) => {
              const price = locale === "ja" ? tier.priceJPY : locale === "en" ? tier.priceUSD : tier.priceHKD;
              ...
              return (
                ...
                >
                  <td className="px-6 py-3 font-medium text-gray-900">
                    ...
                    <span className="text-gray-400 ml-1">{locale === "ja" ? "個" : "個"}</span>
                  </td>
                  <td className="px-6 py-3 text-gray-900">{symbol}{price.toLocaleString()}</td>
                  <td className="px-6 py-3 text-gray-500 text-xs hidden sm:table-cell">{getShippingText(tier.weightKg, locale)}</td>
                  <td className="px-6 py-3 text-gray-600 hidden sm:table-cell">{symbol}{(price * tier.qty).toLocaleString()}</td>
      ...
      {!selectedTier && (
        <div className="px-6 py-4 border-t border-gray-100 text-center">
          <a
            href={waUrl || `https://wa.me/${whatsappNumber || "+8618126380255"}`}
            target="_blank"
            rel="noopener noreferrer"
