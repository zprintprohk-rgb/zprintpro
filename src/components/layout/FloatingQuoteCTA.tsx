...
export function FloatingQuoteCTA({ locale }: { locale: string }) {
  ...
  if (locale === 'zh-hk') {
    ...
    // 2026-08-07 K3 phase-out: 旧 WhatsApp 专用号已退役, 全部统一 198 8085 1334 (call + WhatsApp 同号)
    const waUrl = `https://wa.me/8619880851334?text=${encodeURIComponent(tZh.whatsappMsg + ' ')}`;
    return (
      <div className="hidden lg:flex fixed bottom-24 right-6 z-40 flex-col items-end gap-3">
        {/* WhatsApp 立即查询按钮 (原有, 保留) */}
        <a
          href={waUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-3 bg-[#25D366] hover:bg-[#1FAD52] text-white pl-4 pr-5 py-3.5 rounded-full shadow-2xl hover:scale-105 transition-all"
        ...
        <a
          ...
          className="inline-flex items-center gap-3 bg-[#F87314] hover:bg-[#EA580C] text-white pl-4 pr-5 py-3.5 rounded-full shadow-2xl hover:scale-105 transition-all"
          aria-label={tZh.emailBtn}
        ...
        >
          ...
          <div className="text-left leading-tight">
            <div className="font-bold text-sm">{tZh.emailBtn}</div>
            <div className="text-[10px] opacity-90">{t.microCopy}</div>
          </div>
        </a>
      </div>
    ...
    <div className="hidden lg:flex fixed bottom-24 right-6 z-40 flex-col items-end gap-3">
      ...
      {!open && (
        ...
        >
          ...
          <div className="text-left leading-tight">
            <div className="font-bold text-sm">{tEn.emailBtn}</div>
            <div className="text-[10px] opacity-90">{t.microCopy}</div>
          </div>
        </a>
      )}
