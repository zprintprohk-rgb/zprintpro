...
const categorySubItems: Record<string, string[]> = {
  ...
  'packaging': ['cosmetic-boxes', 'food-boxes', 'mailer-boxes', 'folding-boxes', 'rigid-boxes', 'magnetic-closure-gift-box'],
...
const subItemNames: Record<string, Record<string, string>> = {
  ...
  en: {
    ...
    'cosmetic-boxes': 'Cosmetic Boxes', 'food-boxes': 'Food Boxes',
    ...
    'kraft-paper-packaging-box': 'Kraft Paper Packaging Box', 'gang-run-card-boxes': 'Gang-Run White Card Boxes',
  ...
  ja: {
    ...
    'cosmetic-boxes': '化粧品箱', 'food-boxes': '食品箱',
    ...
    'kraft-paper-packaging-box': 'クラフト包装箱', 'gang-run-card-boxes': '合版ホワイトカードボックス',
...
const featuredDescs: Record<string, Record<string, string>> = {
  'zh-hk': {
    ...
    'cosmetic-boxes': '時尚外觀，品牌加分', 'food-boxes': '食品級材，安全衛生',
  ...
  'en': {
    ...
    'cosmetic-boxes': 'Stylish, brand boost', 'food-boxes': 'Food-grade, hygienic',
    ...
    'kraft-paper-packaging-box': 'Eco kraft, natural texture', 'gang-run-card-boxes': 'No die-cut fee, standard sizes',
  ...
  return (
    <header className="sticky top-0 z-50">
      <div className="max-w-[1320px] mx-auto bg-white shadow-sm">
        {/* Top Bar — 2026-06-07 重排: 删 24/7、WhatsApp 上移、电话/邮箱放大左移 */}
        <div className="bg-gray-50 border-b border-gray-100">
          <div className="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center text-sm sm:text-base py-2.5">
            {/* 左：电话 + 邮箱（放大 + 增强 hover）
            ...
            <div className="flex items-center text-gray-700 font-medium min-w-0">
              ...
              <a href={`mailto:${t.email}`} className="hidden sm:flex items-center gap-2 hover:text-[#2873F5] transition-colors px-4 border-r border-gray-300">
                <Mail className="w-4 h-4" />
                <span>{t.email}</span>
              </a>
            </div>
            {/* 右：WhatsApp (绿, 仅桌面端) + 语言切换
                2026-07-27: 顶部行 WhatsApp 按钮移动端移除 (页面下方已有 WhatsApp 浮动/联系入口, 重复),
                避免移动端把电话号码挤成竖排; 桌面端保持不变 */}
            <div className="flex items-center gap-2 sm:gap-4 shrink-0">
              {/* 2026-06-14 Phase B P0-6: JA locale 在 header 顶部加 LINE 入口
                  - 仅当 NEXT_PUBLIC_LINE_URL 配置后才显示 */}
              {locale === 'ja' && process.env.NEXT_PUBLIC_LINE_URL && (
                <a
                  href={process.env.NEXT_PUBLIC_LINE_URL}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 text-[#06C755] hover:text-[#04A847] font-semibold transition-colors"
              ...
              <a {...getWhatsAppLinkProps(locale, { source: 'header-top', productName: 'Site Header WhatsApp CTA' })} className="hidden lg:flex items-center gap-2 text-green-600 hover:text-green-700 font-semibold transition-colors">
              ...
              <div className="flex items-center gap-1 pl-2 sm:pl-3 border-l border-gray-300 shrink-0 whitespace-nowrap">
                            ...
                            return (
                              <Link key={subSlug} href={`${localePrefix}/product/${subSlug}/`} className="block group flex-1 flex flex-col">
                                <div className="flex-1 relative overflow-hidden rounded-t-lg">
                                  <Image src={getFeaturedImage(subSlug, locale)} alt={getSubItemName(subSlug)} fill className="object-cover object-right group-hover:scale-105 transition-transform" unoptimized loading="lazy" decoding="async" />
