...
export function CategorySidebar({ locale, currentCategorySlug }: CategorySidebarProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 企業批量訂單 + 聯繫方式 */}
      <div className="bg-gradient-to-br from-[#2873F5] to-[#1E5FD1] rounded-lg p-5 md:p-6 text-white">
        <h3 className="font-semibold text-lg mb-2 text-center">{t.enterpriseCta}</h3>
        <p className="text-white/80 text-sm mb-4 text-center">{t.enterpriseDesc}</p>
        <Link
          href={`${localePrefix}/contact/`}
          className="flex items-center justify-center w-full text-[#2873F5] text-sm font-medium bg-white hover:bg-gray-100 px-3 py-2.5 rounded-lg transition-colors mb-3"
        >
          {t.contactUs}
        </Link>
        <a
          ...
          className="flex items-center justify-center gap-2 w-full px-3 py-2.5 border border-white/40 text-white rounded-lg text-sm font-medium hover:bg-white/10 transition-colors mb-4"
        >
        ...
        <div className="text-center pt-1">
          <p className="text-xs text-white/60 mb-1">{t.serviceHours}</p>
