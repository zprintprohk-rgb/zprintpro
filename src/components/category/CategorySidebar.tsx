...
export function CategorySidebar({ locale, currentCategorySlug }: CategorySidebarProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 企業批量訂單 + 聯繫方式 */}
      <div className="bg-gradient-to-br from-[#2873F5] to-[#1E5FD1] rounded-xl p-6 md:p-8 text-white">
        <h3 className="font-bold text-xl mb-3 text-center">{t.enterpriseCta}</h3>
        <p className="text-white/80 text-sm mb-6 text-center leading-relaxed">{t.enterpriseDesc}</p>
        <Link
          href={`${localePrefix}/contact/`}
          className="flex items-center justify-center w-full text-[#2873F5] text-base font-semibold bg-white hover:bg-gray-50 px-4 py-3 rounded-xl transition-colors mb-4 shadow-lg"
        >
          {t.contactUs}
        </Link>
        <a
          ...
          className="flex items-center justify-center gap-2 w-full px-4 py-3 border-2 border-white/50 text-white rounded-xl text-base font-medium hover:bg-white/15 transition-colors mb-6"
        >
        ...
        <div className="text-center pt-2 border-t border-white/20">
          <p className="text-xs text-white/60 mb-1.5">{t.serviceHours}</p>
