...
export function CategorySidebar({ locale, currentCategorySlug }: CategorySidebarProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 企業批量訂單 + 聯繫方式 */}
      <div className="bg-gradient-to-br from-[#2873F5] to-[#1E5FD1] rounded-2xl p-8 md:p-10 pb-12 md:pb-16 text-white">
        <h3 className="font-bold text-2xl mb-4 text-center">{t.enterpriseCta}</h3>
        <p className="text-white/80 text-base mb-8 text-center leading-relaxed">{t.enterpriseDesc}</p>
        <Link
          href={`${localePrefix}/contact/`}
          className="flex items-center justify-center w-full text-[#2873F5] text-lg font-semibold bg-white hover:bg-gray-50 px-4 py-4 rounded-xl transition-colors mb-5 shadow-lg"
        >
          {t.contactUs}
        </Link>
        <a
          ...
          className="flex items-center justify-center gap-2 w-full px-4 py-4 border-2 border-white/50 text-white rounded-xl text-lg font-medium hover:bg-white/15 transition-colors mb-8"
        >
        ...
        <div className="text-center pt-4 border-t border-white/20">
          <p className="text-sm text-white/60 mb-2">{t.serviceHours}</p>
          <p className="text-base font-semibold">{t.hoursValue}</p>
        </div>
      </div>
    </div>
  );
