...
function findBlock(startLabel, nextLabel) {
  ...
  return { start, end: end > 0 ? end : data.length };
}

function expandBlock(startLabel, nextLabel, zhAdd, enAdd, jaAdd) {
  const { start, end } = findBlock(startLabel, nextLabel);
...
expandBlock("export const stickerGuidePillar", "export const flyerGuidePillar",
  '<h2>香港貼紙印刷的創新應用</h2><p>除了傳統的品牌標識與產品標籤，貼紙在香港市場中還有許多創新應用。活動入場手環貼紙能實現快速身份驗證與分區管理；NFC 智能貼紙可讓消費者用手機一觸即達產品資訊頁；防偽貼紙配合全息圖案與微縮文字，能有效打擊假冒產品。對於藝術家與設計師，限量藝術貼紙已成為新興的收藏品類別，智印港支持全彩數碼印刷與特殊工藝，讓創意不受限制。</p><h2>如何選擇可靠的香港貼紙印刷供應商</h2><p>選擇貼紙印刷供應商時，應重點考察四個維度：設備先進性（是否配備 HP Indigo 或 Epson 高精度數碼印刷機）、材料庫存（是否常備多種基材與膠水類型）、打樣能力（能否在 24 小時內提供實物樣品）以及交付可靠性（準時交付率是否達到 95% 以上）。智印港在這四個維度上均達到行業領先水平，為超過 500 家香港本地品牌提供持續穩定的貼紙供應服務。</p>',
...
expandBlock("export const packagingGuidePillar", "export const stickerMaterialsCluster",
  ...
  '<h2>包装の試作とサンプリング</h2><p>量産に着手する前に、包装の試作は構造的完全性、視覚的魅力、ユーザー体験を検証するために不可欠です。智印港は3〜5営業日の迅速な包装試作を提供し、ブランドが最小限のコストで複数の箱構造、紙の厚さ、加工の組み合わせをテストできるようにします。化粧箱には、実際の生産素材を使用した構造モックアップを提供し、クライアントが開封の仕組みや磁石の閉鎖強度を評価できます。組み立て箱には、全面生産前にアートワークの整合性と折りの正確性を確認するフラットダイラインプルーフを提供します。</p><h2>香港ブランドのための持続可能な包装ソリューション</h2><p>持続可能性は香港のブランドにとってもはやオプションではありません。消費者はますます、美学を損なわずに環境影響を最小限に抑えるエコフレンドリーな包装を求めています。智印港は包括的な持続可能なオプションを提供しています：消費後廃棄物100%のリサイクル紙、有害なVOC排出を排除する水性コーティング、柔軟包装用の堆肥化可能な包装フィルムです。また、包装の炭素フットプリント評価も提供し、ブランドが環境への取り組みを定量化して伝えることを支援します。</p>');

// ===================== CLUSTERS =====================
// All clusters need en expansion. Some need zh/ja too.
