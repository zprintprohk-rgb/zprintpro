...
const t1 = audit.slots.filter((s) => s.tier === 'T1_REWRITE').sort((a, b) => (b.gsc?.imps || 0) - (a.gsc?.imps || 0)).slice(0, 30);
const out = t1.map((s) => {
  ...
  return {
    ...
    name: e.name?.[s.locale], minQ: t.minQ, bp: t.bp, bpe: t.bpe, bpj: t.bpj, unit: t.unit, cat: t.cat,
    title: seo.title, h1: seo.h1, desc: seo.description,
    kws: (seo.keywords || []).slice(0, 14),
    issues: s.issues.map((i) => `${i.field}:${i.kind}`),
  };
});
fs.writeFileSync(new URL('../.hermes/_draft-top30.json', import.meta.url), JSON.stringify(out, null, 2));
console.log('slots:', out.length);
for (const o of out) console.log(`${o.slug} | ${o.locale} | imps=${o.imps} pos=${o.pos} | minQ=${o.minQ} bp=${o.bp}/${o.bpe}/${o.bpj} ${o.unit} | ${o.name}`);
