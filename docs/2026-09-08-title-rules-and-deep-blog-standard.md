...
> **数据/规则来源**: K3 9/1 09:54 拍板标题规则 v3（docs/2026-09-01-k3-title-format-rule-v3.md）+ K3 9/3 23:29 拍板 12 铁律（scripts/guards/blog-quality-12-rules-guard.js 机器定义）+ 门童 #12/#13/#15 + K3 8/30「标题长尾 3 筛选」+ v3.3（docs/2026-09-04）+ v4.0 主提示词（9/8 05:06）+ K3 9/9 06:18 v4 写满原则终裁（docs/2026-09-09-k3-title-rule-v4-write-full.md，本文件 §1.5 由此收口）
...
## 1.5 统一口径裁决（✅ 已终裁：K3 9/9 06:18 v4 写满原则）

- v3（9/1 拍板）：「长尾/工艺修饰 1-3 个可堆砌」；
- v4.0 §5.3（9/8 主提示词）：「1 个 GSC 实证长尾钩子，铁律不上第 2 个」；
- **终裁口径（K3 9/9 06:18 v4 写满原则，SSoT = docs/2026-09-09-k3-title-rule-v4-write-full.md）**：**独立长尾词 1-2 个**（均须过 §1.3 三筛选；第 2 个仅在写不满 50 半角当量补词时启用）；**工艺修饰词可 1-3 个**（防水/燙金/異形裁切/DHL 等描述性工艺词，非独立搜索词，不受"第 2 个长尾"限制）。v3「只上 1 个」与 v4.0 §5.3「铁律不上第 2 个」旧口径就此作废。判例依据（8/30 标题 SSoT，stickers「防水抗UV + 異形裁切 + 燙金」三工艺修饰，K3 9/1 评"实在、专业"拍板胜出）保留为史实。
...
| 3-5 | **H2/H3 问句段群** | **(H2+H3)** 总量 6-10 个；其中**问句式 ≥50%**（含边界；消除与门禁实现的分叉，K3 2026-09-19 裁决 iii）（多少/怎樣/如何/how/what/いくら/どう）。**主段锚可为 H2 或 H3**（K3 2026-09-19 决策 方案 a：关键在「层级逻辑清晰」，不强求 H2；外部研究：有清晰 H1-H2-H3 层级的页面被 ChatGPT 引用概率约为无层级页面的 3 倍）；每段正文 ≤3 行 |
...
- ⇒ **答案长度改写口径（K3 2026-09-19 二次修订 → 40-80 词）**：外部实测收敛到单一「甜蜜点」——**40-80 词**是 AI 引擎（Perplexity / ChatGPT）引用率最高的答案长度：该区间提供足够上下文，且写成**可独立理解的完整段落**（不依赖问题本身即可读懂）；多来源确认「直接 Yes/No 开头 + 硬数据点 + 自包含」是标准形态；FAQ 形态对 AI 引用的提升比散文替代方案约高 70-80%。
  **最终口径 = 40-80 词**（下限 = 参考下限，上限 = 甜蜜点上限；上一版 40-120 词上限已作废）。
- ⇒ **B3 FAQ 写做五要件（K3 2026-09-19 收敛，新写/补写一律照此）**：
  ① **格式**：`<p><strong>Qn: 问?</strong><br/>A: 答</p>`（`<p>` 不带 class）—— 写入前必跑 `node .hermes/_probe-pb/precheck-faq-format.mjs`（8 用例含 3 反例）；
  ② **答案长度 40-80 词**；
  ③ **answer-first**：首句 **8-12 词**内给出直接答案（不铺垫）；
  ④ **自包含**：每段答案可独立引用，不依赖上下文/前文；
  ⑤ **数量 5-8 组**聚焦问题（>15 组会稀释特异性；低于 4 组不达段 9 硬线）。
  **依据**（外部实测汇总）：Q&A 结构化格式引用率 34% vs 长文叙事 15%（约 2.3 倍）；明确标注 FAQ 区域 + 约 65% 对话式查询引用；叠加 FAQPage schema 再 +20-25%。
  新写/补写 FAQ（B3 foil 批）按 **40-80 词**执行；**存量已写 FAQ 不动**（churn 红线），仅在字数升级批次顺带改写。
- ⇒ **段 3 口径修订（K3 2026-09-19 决策 方案 a）**：主段锚 **H2 或 H3 均可**，判据 = `(H2+H3) 总数 ≥6 且其中问句式 >50%`（外部研究：问句式 H2/H3 是「成本最低、回报最大」的格式化改动；有清晰 H1-H2-H3 层级的页面被 ChatGPT 引用概率约为无层级页面的 3 倍）。
  - 不选方案 b（把 h3 提升为 h2）：会改动已上线 DOM 结构（churn）并影响锚点/样式。
  - **修订同步三处**（SSoT / cron prompt / 门禁断言 `blog-quality-12-rules-guard.js` 段 3），并由门童 #21 重新绑定 sha256（见 §7.2）。
  - ⚠️ **修订后实测暴露的真实内容缺口（不是门禁 bug，待授权修）**：`poster-printing-guide` 的 **en/ja 用 `<h3>` 载**但**标题全为陈述式**（`1. Common Poster Sizes` / `1. ポスターの主なサイズ` …），**问句式 = 0**；zh-hk 同篇有 **9 个问句式 H2** —— 属**三语结构对齐缺口**（zh-hk 20,043 字 / en 12,659 / ja 9,200）。修法 = 把 en/ja 段锚改写为问句式（对齐 zh-hk 问句集），属内容改动，**需指令包授权**。
