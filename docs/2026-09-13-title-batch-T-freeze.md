...
| 3 | **title 字符数口径**：「对齐要补齐足够的字符数。**更新到 58 字符数内就可以**。标题的字符数很宝贵的」 | 目标区 **50–58 半角当量**（取代 v4 的 50–54）；T2-1b 提案表已出（`docs/2026-09-13-title-batch-T2-1b-proposals.md`）<br>🔻 **已由 2026-09-19 裁决收窄为 50–57, 见下方取代说明** |
| 4 | **页面 title 与 JSON-LD headline 的一致性**：等 T2 完成后自然对齐 | T1 已把 blog 内嵌 `headline` 单品牌化；对应页面 title 属 T2，完成后一致 |

### §6-3 取代说明（K3 2026-09-19 裁决：58 为**阻断线**）

> **本节历史正文保留不改写**（per §0.34.2）。以下为收窄后的现行口径。

| 项 | 2026-09-13 原口径 | **2026-09-19 现行口径** |
|---|---|---|
| 目标区 | 50–58（含 58 视为达标） | **50–57** |
| 58 的定性 | 达标上限 | **阻断线（硬拦）** |
| 口径实现 | `scripts/guards/title-equiv.js` `TITLE_MAX = 58` | `TITLE_MAX = **57**` |

**裁决理由（K3）**：① 行业实证 51–60 字符区间重写率最低，58–60 已在该区间上沿；
② **规则一致性优先** —— 书面口径与门禁必须同源，否则「58 到底合不合规」长期矛盾，
每个批次都要临场判断（2026-09-19 人工批 3 条初稿恰落 58，被迫临场决策即为实例）。

**改动实测影响**：当量 == 58 的槽位 **8 条**由 OK 转 TRIM（其中 5 条为 en `Free Shipping $99+`
站级模板）⇒ 违规总数 50 → 58。这 8 条按常规批次流程处理，不回溯已进验证窗的批次。

**同步落点**：`scripts/guards/title-equiv.js`（代码）+ 本说明（书面）—— 二者同源，不再分叉。
`AGENTS.md` §5 与 `docs/2026-09-09-…-v4-write-full.md` 的取代横幅亦已指向本口径。
